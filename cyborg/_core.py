...
@command(pattern=("load (?P<shortname>\w+)$"))  # pylint:disable=E0602
...
@command(pattern=("dbload (?P<shortname>\w+)$"))  # pylint:disable=E0602
async def load_reload(event):
    await event.delete()
    shortname = event.pattern_match["shortname"]
    try:
...
@command(pattern=("(?:dbunload|dbremove) (?P<shortname>\w+)$"))  # pylint:disable=E0602
...
@command(pattern=("(?:unload|remove) (?P<shortname>\w+)$"))  # pylint:disable=E0602
async def remove(event):
    await event.delete()
    shortname = event.pattern_match["shortname"]
    if shortname == "_core":
...
@command(pattern=("send (?P<shortname>\w+)$"))  # pylint:disable=E0602
...
@command(pattern=("install"))  # pylint:disable=E0602
